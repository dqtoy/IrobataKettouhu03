...
public class SoundController : SingletonMonoBehaviour<SoundController> {
	...
	private const int SE_SOURCE_NUM = 0;
	//全AudioClipを保持
	...
	public static float setloopDefine{	set{loopTime=value;}}
	public static float setendDefine{	set{endTime=value;}}
...
//BGM止めないフラグ
 //   public bool DontDestroyEnabled = true;
	...
	/// </summary>
	public void PlayBGM (string bgmName, float fadeSpeedRate)
	{
...
*/
    ...
    void Update () {
		...
		if (_bgmSource != null && _bgmSource.isPlaying) {
			if (_bgmSource.time >= endTime) {
				_bgmSource.time = loopTime;
			}
		}
		...
		if (_bgmSource.volume <= 0) {
			...
			if (!string.IsNullOrEmpty (_nextBGMName)) {
				PlayBGM (_nextBGMName,BGM_FADE_SPEED_RATE_HIGH);
			}
		}
    }
